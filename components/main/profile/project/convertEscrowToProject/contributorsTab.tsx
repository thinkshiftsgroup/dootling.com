...
import ConfirmationModal from "@/components/main/modal/confirmationModal";
...
const ContributorsTab = ({ project }: any) => {
  ...
  const handleOpenModal = (id: string, name: string) => {
    setSelectedContributor({ id, name });
    setOpenModal(true);
  };

  const handleRemoveClick = (userId: string, userName: string) => {
    setSelectedContributor({ id: userId, name: userName });
    setShowModal(true);
  };
    ...
    <div className="overflow-x-auto my-5">
              ...
              <tr key={i} className="hover:bg-[#E7F1FF]/10">
                ...
                <td className="py-3 px-2">
                  <div className="flex items-center justify-center gap-2">
                    <span
                      onClick={() =>
                        handleOpenModal(cont.id, cont.user.fullName)
                      }
                    ...
                    <span
                      className="cursor-pointer"
                      onClick={() =>
                        handleRemoveClick(cont.id, cont.user.fullName)
                      }
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
      ...
      {showModal && selectedContributor && (
        <ConfirmationModal
          ...
          message={`Are you sure you want to remove contributor ${selectedContributor.name}?`}
