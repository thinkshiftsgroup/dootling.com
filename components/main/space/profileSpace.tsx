...
interface SpacesCardProps extends Spaces {}
...
interface SpacesDashboardProps {
  Spaces?: Spaces[];
  invited?: Spaces[];
  peerConfirmation?: Spaces[];
}
 
const SpacesCard: React.FC<SpacesCardProps> = ({
  imageSrc,
  isPrivate,
  projectName,
  collaborators,
...
const SpacesDashboard: React.FC<SpacesDashboardProps> = ({
  Spaces = [],
}) => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          <div className="w-full space-y-1">
            {Spaces.map((Spaces) => (
              <SpacesCard key={Spaces.id} {...Spaces} />
            ))}
          </div>
        </div>
      </div>

      <AddSpaceModal open={showModal} onClose={() => setShowModal(false)} />
    </div>
  );
};
