...
const ProjectTasks = ({ projectId }: any) => {
  ...
  const { getAllProjectById, getMilestonebyId, getTasksbyId } = useProject();
  ...
  const { data: milestoneData, isLoading: milestoneLoading } =
    getTasksbyId(selectedMilestone);
    ...
    <div className="flex-1 w-full p-2 overflow-y-scroll hide-scrollbar space-y-4">
      ...
      <div className="w-1/2">
        <select
          ...
          name=""
          id=""
          className=" text-sm sm:!text-sm p-1.5 rounded-sm w-full bg-white border border-[#157bff]"
        >
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse">
          ...
          <tbody className="[&>tr:nth-child(even)]:bg-[#E7F1FF]/30">
            {selectedMilestone && milestoneData?.tasks?.length > 0 ? (
              milestoneData.tasks.map((task: any) => (
                <>
                  <tr key={task.id} className="hover:bg-[#E7F1FF]/10">
                    ...
                    <td className="py-3 px-2 text-gray-600 text-sm text-left">
                      ...
                      >
                        {expandedRow === task.id ? "show less" : "show more"}
                      </span>
                    </td>
              ...
              <tr>
                <td
                  colSpan={8}
                  className="text-center text-gray-500 py-5 text-sm italic"
                >
                  {selectedMilestone
